...
//                                  |    |   |                     после которого станет видимым
    ...
    private Quest__ (final char code, final int dx2, final int dy2, final int ammount, final Material icon, final String parent, final String displayName, final String description, final int pay) {
        this.code = code;
    ...
    private static final Map<Character,Quest__> codeMap;
    private static final Map<String,Quest__> nameMap;
    
    static {
        Map<Character,Quest__> c = new ConcurrentHashMap<>();
        Map<String,Quest__> n = new CaseInsensitiveMap<>();
        //Map<Quest,List<String>> l = new ConcurrentHashMap<>();
        for (final Quest__ d : Quest__.values()) {
            c.put(d.code,d);
            n.put(d.name(),d);
    ...
    public static Quest__ byCode(final char code){
        return codeMap.get(code);//intMap.containsKey(tag) ? intMap.get(tag) : EMPTY;
    }    
    
    public static Quest__ byName(final String name){
    ...
    public static EnumSet<Quest__> getChidren(String parentName) {
        //parentName = parentName.toLowerCase();
        final EnumSet<Quest__> result = EnumSet.noneOf(Quest__.class);
        for (final Quest__ q : Quest__.values()) {
